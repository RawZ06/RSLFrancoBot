 ...
 */
@Component
public class SaladSongsButtonHandler {

    private final SeedService seedService;
    private final SeedPresenter presenter;

    public SaladSongsButtonHandler(SeedService seedService, SeedPresenter presenter) {
    ...
    public void handle(DiscordInteraction interaction) {
        try {
            ...
            SeedResult result = seedService.generateSeed(
                    SeedMode.SALAD_SONGS,
                    interaction.getUserId(),
                    Map.of()
            );

            // Send final result as channel message (persists after cleanup)
            interaction.sendChannelMessage(presenter.presentSeedResult(result, "Chants en folie", interaction.getUsername()));
