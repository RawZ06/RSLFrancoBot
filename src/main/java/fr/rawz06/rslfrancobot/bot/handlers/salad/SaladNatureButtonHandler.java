 ...
 */
@Component
public class SaladNatureButtonHandler {

    private final SeedService seedService;
    private final SeedPresenter presenter;

    public SaladNatureButtonHandler(SeedService seedService, SeedPresenter presenter) {
    ...
    public void handle(DiscordInteraction interaction) {
        try {
            ...
            SeedResult result = seedService.generateSeed(
                    SeedMode.SALAD_NATURE,
                    interaction.getUserId(),
                    Map.of()
            );

            // Send final result as channel message (persists after cleanup)
            interaction.sendChannelMessage(presenter.presentSeedResult(result, "Nature en folie", interaction.getUsername()));
